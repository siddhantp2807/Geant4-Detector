...
G4bool MySensitiveDetector::ProcessHits(G4Step* aStep, G4TouchableHistory* ROhis)
{
    ...
    // Using Analysis Manager to fill ntuple with position and energy deposition data
    G4AnalysisManager *man = G4AnalysisManager::Instance();
    man->FillNtupleIColumn(0, evt);
    man->FillNtupleDColumn(1, posDetector[0]);
    man->FillNtupleDColumn(2, posDetector[1]);
    man->FillNtupleDColumn(3, posDetector[2]);
    man->FillNtupleDColumn(4, energyDeposition); // Add energy deposition to ntuple
    man->AddNtupleRow(0);
